...
macro_rules! hrtim_cr_helper {
    (HRTIM_MASTER: $cr_type:ident:
        ...
        hrtim_cr_helper!(HRTIM_MASTER: $cr_type: $cmpXr, [$(($Trigger: $trigger_bits)),*], [$(($event_dst, $tim_event_index)),*]);
    };
    ...
    ($TIMX:ident: $cr_type:ident:
        ...
        impl<PSCL> HrCompareRegister for $cr_type<$TIMX, PSCL> {
            fn get_duty(&self) -> u16 {
                ...
                tim.$cmpXr().read().cmp().bits()
            }
            fn set_duty(&mut self, duty: u16) {
                let tim = unsafe { &*$TIMX::ptr() };

                tim.$cmpXr().write(|w| unsafe { w.cmp().bits(duty) });
            }
        }
